...
const createRepo = (token, name) => {
  ...
  let data = {
    ...
    auto_init: true,
    description:
      'Collection of LeetCode questions to ace the coding interview! - Created using [CodeHub](https://github.com/Adnan-Umar/CodeHub)',
  };
...
const linkStatusCode = (status, name) => {
  ...
  switch (status) {
    case 301:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to CodeHub. <br> This repository has been moved permenantly. Try creating a new one.`,
      );
      $('#error').show();
      break;
    ...
    case 403:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to CodeHub. <br> Forbidden action. Please make sure you have the right access to this repository.`,
      );
      $('#error').show();
      break;
    ...
    case 404:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to CodeHub. <br> Resource not found. Make sure you enter the right repository name.`,
      );
      $('#error').show();
      break;
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          chrome.storage.local.set({ mode_type: 'commit', repo: res.html_url }, () => {
            $('#error').hide();
            $('#success').html(
              `Successfully linked <a target="blank" href="${res.html_url}">${name}</a> to CodeHub. Start <a href="https://leetcode.com">LeetCoding</a> now!`,
            );
            $('#success').show();
            $('#unlink').show();
          });
    ...
    */
    chrome.storage.local.get('codehub_token', data => {
      ...
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant CodeHub access to your GitHub account to continue (launch extension to proceed)',
        );
        $('#error').show();
        $('#success').hide();
      } else if (option() === 'new') {
  ...
  if (mode && mode === 'commit') {
    ...
    chrome.storage.local.get('codehub_token', data2 => {
      ...
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant CodeHub access to your GitHub account to continue (click CodeHub extension on the top right to proceed)',
        );
        $('#error').show();
        $('#success').hide();
        /* Hide accordingly */
        ...
        chrome.storage.local.get('codehub_hook', repoName => {
          ...
          if (!hook) {
            /* Not authorized yet. */
            $('#error').text(
              'Improper Authorization error - Grant CodeHub access to your GitHub account to continue (click CodeHub extension on the top right to proceed)',
            );
            $('#error').show();
            $('#success').hide();
            /* Hide accordingly */
